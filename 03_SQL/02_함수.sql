...
select emp_name, upper(emp_name) as upper, lower(emp_name) as lower, char_length(emp_name) as length
from emp ;

-- 직원 이름(emp_name) 의 자릿수를 15자리로 맞추고 15자가 안되는 이름의 경우  공백을 앞에 붙여 조회. 
select lpad(emp_name, 15, ' ') as name,
		rpad(emp_name, 15, ' ') as name2,
		char_length(lpad(emp_name, 15, ' ')) as '글자수'
        
from emp;
    
--  EMP 테이블에서 이름(emp_name)이 10글자 이상인 직원들의 이름(emp_name)과 이름의 글자수 조회
select emp_name, char_length(emp_name) as length
from emp
where char_length(emp_name) >= 10;

select concat('$' , salary) as salary from emp;
select trim('a');
...
select emp_id, emp_name,
		salary * 1.15 as 'sal_raise',
		floor(salary - (salary * 1.15)) as 'diff'
from emp;
...
select emp_name,
		subdate(hire_date, interval 6 month) as "입사 6개월 전",
        hire_date as "입사일",
...
from emp
where emp_id = 200;
# %y %m %d %H 시 %i %s %w %p 오전오후

--  각 직원의 이름(emp_name), 근무 개월수 (입사일에서 현재까지의 달 수)를 계산하여 조회. 근무개월수 내림차순으로 정렬.
select emp_name, timestampdiff(month, hire_date, curdate()) as "근무개월수",
		timestampdiff(year, hire_date, curdate()) as "근무년수"
from emp
order by 2 desc;

-- 250 개월 이상 근무한 직원 정보를 조회
select *
from emp
where timestampdiff(month, hire_date, curdate()) >= 250;
...
select emp_id, emp_name, salary,
		ifnull(salary * comm_pct, '0') as '커미션'
from emp;
...
select salary,
       case when salary >= 10000 then '1등급' end
          --  when salary < 10000 then '2등급' end as "등급"
-- if(salary >= 10000, '1등급', '2등급')
from emp;
...
from emp
where job in ('AD_PRES', 'FI_ACCOUNT', 'PU_CLERK', 'IT_PROG', 'AD_VP');
...
select emp_id, emp_name, salary,
	   case when salary <5000 then 0.3 * salary
            when salary between 5000 and 9999.999 then 0.2 * salary
            when salary >=10000 then 0.1*salary end as "급여인상액",
from emp;