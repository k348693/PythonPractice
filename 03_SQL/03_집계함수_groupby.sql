...
select sum(salary), round(avg(salary), 2), -- min max 
	   round(stddev(salary),2), round(variance(salary),2)
from emp;

# select * from emp
# where salary > avg(salary)  --  where 절에는 직계함수를 직접 사용불가
...
select dept_name, avg(salary), count(*) as "직원수"
from emp
group by dept_name
